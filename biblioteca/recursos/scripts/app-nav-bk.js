$(function () {
  ...
  const $botones = $nav.find('.barra-lateral-enlace--boton');
  ...
  function abrirCon($btn) {
    ...
    if ($liPersistente && !$btn.is($liPersistente.find('.barra-lateral-enlace--boton'))) {
  ...
  // eventos
  $nav.on('click', '.barra-lateral-enlace--boton', function (e) {
    e.preventDefault();
    const $btn = $(this);
  ...
  $nav.on('keydown', '.barra-lateral-enlace--boton', function (e) {
    const $btn = $(this);
    ...
    if ($linkSub.length) {
      ...
      if ($ulPadre.length) {
        ...
        if ($btnPadre.length) {
          ...
          $btnPadre.addClass('activo seccion-activa activo-persistente'); // marca visual
    ...
    if ($(window).width() <= PUNTO_CORTE_MOVIL) {
      ...
      if ($ul.hasClass('is-accordion-open')) {
        ...
        $('.barra-lateral-submenu.is-accordion-open').each(function () {
          ...
          $nav.find('.barra-lateral-enlace--boton[aria-expanded="true"]').not($btn).each(function () {
